...
pillar db seed                             # run the project seed runner (production-guarded)

# Preview any command — prints the exact argv, cwd, destructive/applies flags, and the SQL diff
pillar db migrate --name add_role --preview
pillar db deploy --preview
pillar db seed --preview
```
...
- **Prisma**: prints the SQL diff of the next migration via `prisma migrate diff --script` (best-effort — falls back silently if the shadow DB isn't configured).
- **Drizzle**: prints the contents of every `.sql` file in the drizzle out dir (default `drizzle/`, override with `PILLAR_DRIZZLE_OUT`) in journal order. `drizzle-kit migrate` will apply any not yet recorded in `__drizzle_migrations`.
- **TypeORM**: prints the contents of every migration file under `src/migrations/` (override with `PILLAR_TYPEORM_MIGRATIONS`). The full file body is shown since the SQL lives inside `queryRunner.query(...)` calls.
- Output is capped at ~8 KB per file and ~32 KB total; overflow is marked inline.

**Seeding** (`pillar db seed`):

- Runs the project seed runner generated by `pillar seed generate <resource>` (`src/seeds/run.ts` for TypeScript projects, `.js` otherwise).
- Treated as destructive (seeds insert data), so the same `NODE_ENV=production` guard applies — pass `--force-production` to override.
- `--preview` prints the argv (`npx tsx src/seeds/run.ts` or `node src/seeds/run.js`) without executing.
- If no runner exists, the command hints at `pillar seed generate` rather than silently doing nothing.
...
| `pillar db <op>` | Database migrations — `generate`, `migrate`, `deploy`, `status`, `reset`, `rollback`, `seed` (Prisma/Drizzle/TypeORM; SQL preview everywhere) |
